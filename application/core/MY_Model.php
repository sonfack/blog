...
class MY_Model extends CI_Model{
	...
	protected $_etat = 'status';  // l etat 1 correspond a l etat actif et 0 (zero) etat inactif
    ...
    /*
     * data_validate($data) permet de valider les donnee avant des les stocker dans la BD
     * 
     */
	...
	public function get_or_record($cond, $id_produit = null){
		if(count($cond) >=2){
			$i = 0;
			foreach($cond as $val){
				if($i == 0){
					...
					$this->db->or_where(array($val[0] => $val[1]));
				}
				if($id_produit != null){
